...
def plot_diagnostic(points, sol_t, sol_f, n, pen, path):
    p = np.array(points)
    fig, ax = plt.subplots()
    ax.set_xlim(T_0, T_f); ax.set_ylim(0.0, 1.0)
    ax.set_xlabel("Time $t$"); ax.set_ylabel("Fidelity $F(t)$")
...
def main(out_dir: Path):
    ...
    for pen, ns in runtime.items():
        ...
        for n in ns:
            ...
            try:
                ...
                rows.append({
                    "n": n, "penalty": pen, "F_thr": F_thr,
                    "nsteps": nsteps, "T_0": T_0, "T_f": T_f,
                    "t_star": sol_t, "F_at_t_star": sol_f, "n_evals": len(points),
                })
    ...
    fig.savefig(main_fig_path, bbox_inches="tight"); plt.close(fig)
    ...
    def make_rep_panel(n: int, pens, path: Path):
        ...
        for ax in axes:
            ax.set_xlabel("Time $t$"); ax.set_xlim(T_0, T_f); ax.set_ylim(0.0, 1.0)
